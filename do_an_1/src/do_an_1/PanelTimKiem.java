...
public class PanelTimKiem extends JPanel {
    ...
    private void timKiem() {
        ...
        if (type.equals("Sản phẩm")) {
            ...
            List<Object[]> dataList = new ArrayList<>();
            for (KhachHang kh : customerManager.getDsKhachHang()) {
            ...
            for (HoaDon hd : invoiceManager.getDsHoaDon()) {
    ...
    private void hienThiChiTietKhachHang(String maKH) {
        ...
        KhachHang kh = customerManager.getDsKhachHang().stream()
        ...
        List<Object[]> dataList = new ArrayList<>();
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy");
        for (HoaDon hd : invoiceManager.getDsHoaDon().stream().filter(h -> h.getMaKH().equals(maKH)).collect(Collectors.toList())) {
    ...
    private void hienThiChiTietHoaDon(String maHD) {
        ...
        for (HoaDon hd : chiTietHD) {
            String tenSP = productManager.getDsSanPham().stream()
                    ...
                    .map(SanPham::getTen)
                    .orElse(hd.getTenSP());
            double gia = productManager.getDsSanPham().stream()
                    .filter(sp -> sp.getMaSP().equals(hd.getTenSP()))
                    .findFirst()
                    .map(SanPham::getGia)
